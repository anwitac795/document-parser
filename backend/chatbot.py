...
# --- Community chat (WebSocket + MongoDB) ---
...
def _require_mongo_db():
    if mongo_db is None:
        raise HTTPException(status_code=500, detail="MongoDB is not configured or not connected")
    return mongo_db
...
async def _startup_mongo():
    ...
    try:
        ...
        # Ensure indexes
        await mongo_db["community_chats"].create_index("communityId", unique=True)
        await mongo_db["community_messages"].create_index([("communityId", 1), ("createdAt", -1)])

    except Exception as e:
        # Keep the API running for non-Mongo endpoints, but community chat will fail with 500.
        mongo_client = None
        mongo_db = None
...
def _parse_before_datetime(before: str) -> datetime:
    """Parse ISO datetime used for pagination."""
    # Accept both with/without timezone; treat naive as UTC.
    try:
        cleaned = before.replace("Z", "+00:00")
        dt = datetime.fromisoformat(cleaned)
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        return dt
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid 'before' timestamp. Use ISO format, e.g. 2025-01-01T00:00:00Z")


@app.get("/api/communities/{community_id}/messages")
async def get_community_messages(
    community_id: str,
    limit: int = Query(50, ge=1, le=200),
    before: Optional[str] = None,
):
    """Fetch chat history for a community (MongoDB)."""
    db = _require_mongo_db()

    query: Dict[str, Any] = {"communityId": community_id}
    if before:
    ...
    messages = [
        {
            ...
            "createdAt": doc.get("createdAt").astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
...
class CommunityConnectionManager:
    ...
    async def connect(self, community_id: str, websocket: WebSocket) -> None:
        ...
        self._rooms.setdefault(community_id, set()).add(websocket)
    ...
    async def broadcast(self, community_id: str, message: Dict[str, Any]) -> None:
        room = self._rooms.get(community_id, set())
        dead: List[WebSocket] = []
        for ws in list(room):
            try:
...
async def _ensure_community_chat_doc(
    db,
    community_id: str,
...
@app.websocket("/ws/communities/{community_id}")
async def ws_community_chat(websocket: WebSocket, community_id: str):
    """Realtime community group chat via WebSockets."""
    await community_ws_manager.connect(community_id, websocket)
    ...
    try:
        ...
        if not isinstance(hello, dict) or hello.get("type") != "hello":
            await websocket.send_json({"type": "error", "error": "First message must be {type: 'hello'}"})
            return

        user_id = str(hello.get("userId") or "").strip()
        if not user_id:
            await websocket.send_json({"type": "error", "error": "userId is required"})
            return

        user_name = str(hello.get("userName") or hello.get("displayName") or "").strip() or None

        # Lazy create community chat doc (with createdAt)
        await _ensure_community_chat_doc(
            db,
            community_id,
            name=hello.get("communityName"),
            image_url=hello.get("communityImage") or hello.get("imageUrl"),
            description=hello.get("communityDescription"),
        )
        ...
        while True:
            ...
            if msg_type == "typing":
                await community_ws_manager.broadcast(
                    community_id,
                    {
                        "type": "typing",
                        "communityId": community_id,
            ...
            if not content:
                continue
            if len(content) > 4000:
                await websocket.send_json({"type": "error", "error": "Message too long (max 4000 chars)"})
                continue

            now = datetime.now(timezone.utc)

            # Ensure community doc exists and update updatedAt
            await _ensure_community_chat_doc(db, community_id)
            ...
            res = await db["community_messages"].insert_one(insert_doc)
        ...
        community_ws_manager.disconnect(community_id, websocket)
